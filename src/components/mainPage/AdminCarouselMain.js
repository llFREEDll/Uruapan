...
const AdminCarouselMain = () => {
    ...
    const PostCarousel = async() =>{
        ...
        await axios.post(urls.carouselMain,form,{headers:{'content-type':'multipart/form-data'}})
        .then(response =>{
    ...
    const PutCarousel = async() =>{
        ...
        //console.log(form);
        await axios.post(urls.carouselMain,form,{params: {id: Number(itemSeleccionado.id) }})
        .then(response =>{
            var newData = data;
            newData.map(item=>{
    ...
    const DeleteCarousel=async()=>{
        ...
        await axios.post(urls.carouselMain, f, {params: {id: Number(itemSeleccionado.id) }})
        .then(response=>{
    ...
    return (
        <>
            <div className ="container text-center">
                <h1>Carrusel Principal Uruapan <button className = "btn btn-success m-5" onClick = {()=>OpenCLoseModalInsertar()}>Nueva Imagen de Carrusel</button></h1>
                {data !== [] ?
                <>
...
export default AdminCarouselMain